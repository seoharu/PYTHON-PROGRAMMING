...
META_PATH = Path("mask_transformation/ImageNetS919/annotation_hstack_colormap_metadata.csv")
VARIANT_ROOT = Path("mask_transformation/ImageNetS919_single_object_variants_white_mask")

OUT_DIR = Path("AE/ImageNetS919_AE_latent_sphere_white_mask/results")
LATENT_DIR = OUT_DIR / "latents"

EXTERNAL_REPO_ROOT = Path("AE/external/imagenet-autoencoder")
AE_CHECKPOINT_PATH = Path("AE/external/imagenet-autoencoder/checkpoints/imagenet-vgg16.pth")
ARCH = "vgg16"
...
def compute_metrics(z_orig, z_var):
    ...
    cos = F.cosine_similarity(z_orig.unsqueeze(0), z_var.unsqueeze(0), dim=1).item()
...
def main():
    ...
    valid_rows = [r for r in rows if r["original_exists"] and r["variant_exists"]]
    missing_rows = [r for r in rows if not (r["original_exists"] and r["variant_exists"])]

    pd.DataFrame(rows).to_csv(OUT_DIR / "pairs_all.csv", index=False)
    pd.DataFrame(missing_rows).to_csv(OUT_DIR / "missing_pairs.csv", index=False)
    ...
    try:
        for idx, row in enumerate(valid_rows):
            ...
            try:
                ...
                orig_latent_path = latent_path_for(class_id, image_id, "original")
                var_latent_path = latent_path_for(class_id, image_id, variant)
                ...
                print(f"[WARN] failed row {idx} {row['image_id']} {row['variant']}: {e}", flush=True)
            ...
            if (idx + 1) % 100 == 0:
                ...
                print(f"[PROGRESS] {idx + 1}/{len(valid_rows)}", flush=True)
