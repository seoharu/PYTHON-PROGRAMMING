...
DEFAULT_META_PATH = Path("mask_transformation/ImageNetS919/annotation_hstack_colormap_metadata.csv")
DEFAULT_VARIANT_ROOT = Path("mask_transformation/ImageNetS919_single_object_variants_white_mask")
DEFAULT_OUT_DIR = Path("AE/ImageNetS919_AE_latent_sphere_white_mask/results_optimized")
DEFAULT_EXTERNAL_REPO_ROOT = Path("AE/external/imagenet-autoencoder")
DEFAULT_AE_CHECKPOINT_PATH = Path("AE/external/imagenet-autoencoder/checkpoints/imagenet-vgg16.pth")
...
def resolve_device(device_name: str) -> str:
    return device_name if device_name == "cuda" and torch.cuda.is_available() else "cpu"
...
def variant_path_for(config: LatentAnalysisConfig, class_id: str, image_id: str, variant: str) -> Path:
    ext = ".png" if variant in PNG_VARIANTS else ".jpg"
    return config.variant_root / variant / class_id / f"{image_id}{ext}"


def latent_path_for(config: LatentAnalysisConfig, class_id: str, image_id: str, condition: str) -> Path:
...
def iter_pair_rows(config: LatentAnalysisConfig) -> Generator[dict[str, Any], None, None]:
    ...
    for _, r in df.iterrows():
        ...
        for variant in config.variants:
            variant_path = variant_path_for(config, class_id, image_id, variant)
            yield {
                "class_id": class_id,
                "image_id": image_id,
                "variant": variant,
...
def pair_key(row: dict[str, Any]) -> tuple[str, str, str]:
    return (str(row["class_id"]), str(row["image_id"]), str(row["variant"]))
...
def load_done_keys(metric_csv: Path) -> set[tuple[str, str, str]]:
    if not metric_csv.exists():
        return set()
    try:
        df = pd.read_csv(metric_csv, usecols=["class_id", "image_id", "variant"])
    except Exception:
        return set()
    return {(str(r.class_id), str(r.image_id), str(r.variant)) for r in df.itertuples(index=False)}
...
def get_original_latent(
    ...
    path = latent_path_for(config, row["class_id"], row["image_id"], "original")
...
def get_variant_latent(
    ...
    path = latent_path_for(config, row["class_id"], row["image_id"], row["variant"])
    if config.use_disk_cache and path.exists():
...
def summarize(config: LatentAnalysisConfig) -> None:
    ...
    variant_summary = df.groupby("variant")[metric_cols].agg(["mean", "std", "median", "min", "max"])
    variant_summary.columns = ["_".join(c) for c in variant_summary.columns]
    variant_summary = variant_summary.reset_index()

    overall_summary = df[metric_cols].agg(["mean", "std", "median", "min", "max"]).T.reset_index()
    overall_summary.columns = ["metric", "mean", "std", "median", "min", "max"]

    variant_summary.to_csv(config.out_dir / "variant_summary.csv", index=False)
    overall_summary.to_csv(config.out_dir / "overall_summary.csv", index=False)
...
def run_pipeline(config: LatentAnalysisConfig) -> RuntimeStats:
    ...
    transform = build_eval_transform(config.resize_size, config.crop_size)
    ...
    if done_keys:
        print(f"[INFO] resume: {len(done_keys)} done pair keys loaded", flush=True)

    for row in iter_pair_rows(config):
        stats.total_pairs += 1
        append_csv_row(config.pairs_all_csv, list(row.keys()), row)

        if not (row["original_exists"] and row["variant_exists"]):
            stats.missing_pairs += 1
            append_csv_row(config.missing_pairs_csv, list(row.keys()), row)
            continue
        ...
        try:
            z_orig = get_original_latent(config, encoder, transform, row, device, original_cache, stats)
            z_var = get_variant_latent(config, encoder, transform, row, device, stats)
            metric_dict = compute_metrics(z_orig, z_var)

            orig_latent_path = latent_path_for(config, row["class_id"], row["image_id"], "original")
            var_latent_path = latent_path_for(config, row["class_id"], row["image_id"], row["variant"])
            ...
            append_csv_row(config.metric_csv, FIELDNAMES, output_row)
            done_keys.add(key)
            stats.metric_rows_written += 1

        except Exception as e:
            stats.failed_pairs += 1
            failed_row = {**row, "error": repr(e)}
            append_csv_row(config.failed_pairs_csv, list(failed_row.keys()), failed_row)
            print(f"[WARN] failed {key}: {e}", flush=True)

        if stats.valid_pairs > 0 and stats.valid_pairs % config.progress_every == 0:
            print(
                f"[PROGRESS] valid={stats.valid_pairs}, written={stats.metric_rows_written}, "
                f"skipped={stats.skipped_pairs}, missing={stats.missing_pairs}, failed={stats.failed_pairs}",
                flush=True,
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Optimized AE latent metric pipeline for ImageNetS919 variants.")
    ...
    parser.add_argument("--external-repo-root", type=Path, default=DEFAULT_EXTERNAL_REPO_ROOT)
    parser.add_argument("--checkpoint-path", type=Path, default=DEFAULT_AE_CHECKPOINT_PATH)
